"""
Module để download video từ Instagram
"""
...
def download_instagram(url, output_path="downloads"):
    """
    Download video từ Instagram
    
    Args:
        url: URL của video Instagram
    ...
    try:
        with yt_dlp.YoutubeDL({'quiet': True}) as ydl:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl_download:
                print(f"Đang download video từ Instagram: {url}")
        ...
        print(f"Lỗi khi download video Instagram: {e}")
        raise
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Cách sử dụng: python instagram.py <url>")
        sys.exit(1)
    download_instagram(sys.argv[1])
